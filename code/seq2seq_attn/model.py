...
def compute_attention(attention_mechanism, cell_output):
    alignments, _ = attention_mechanism(cell_output, None)
...
class Seq2SeqAttn(object):
    ...
    def build_graph(self):
        ...
        with self.graph.as_default():
            ...
            with tf.variable_scope('decoding', reuse = tf.AUTO_REUSE) as vs:
                ...
                attn_mechanism = MyBahdanauAttention(opts.attn_depth,
                    memory = enc_outputs,
                    memory_sequence_length = tiled_enc_input_len) # alpha
                ...
                if opts.mode == 'TRAIN':
                    ...
                    outputs_dec, _ = tf.nn.dynamic_rnn(cell = cell_dec,
                        ...
                        scope = vs)
                    # decoder的结果？
                    # logits: vector (batch * sequence size * vector size)
                    logits = output_layer.apply(outputs_dec)
                    # batch size * max sentence length; binary
    ...
    def train(self, train_set, save_path, restore_epoch, valid_set = None):
        ...
        for epoch in range(opts.num_epochs):
            ...
            if valid_set is not None:
                valid_ppl.append(self.validate(valid_set))
    ...
    def predict(self, enc_input, enc_input_len):
